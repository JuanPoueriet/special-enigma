...
|---|---|---|---|---|---|
| api-gateway | Edge/API | Federado + proxy | Platform API | timeout, retry, circuit breaker, kill switch | 99.99%, p99 < 120ms |
| api-fiscal | Fiscal | Microservicio dedicado (Kafka + HTTP) | Fiscal Platform | timeout, retry idempotente, DLQ, kill switch país/provider | 99.95%, error rate < 1% |
| api-billing | Billing | Microservicio + eventos | Billing Core | outbox, retry con idempotency key, reconciliación diaria | 99.95% |
