...
export class StepAccountInfo implements OnInit {
  ...
  ngOnInit() {
    if (this.group) {
      ...
      if (emailControl) {
        emailControl.addAsyncValidators(
          AsyncValidators.createEmailValidator(this.http, this.apiUrl),
        );
  ...
  getErrorMessage(controlName: string): string {
    ...
    if (control?.touched && control?.errors) {
      ...
      if (control.errors['strongPassword'])
        return 'REGISTER.ERRORS.PASSWORD_WEAK';
      if (control.errors['passwordMismatch'])
        return 'REGISTER.ERRORS.PASSWORD_MISMATCH';
    }
    return '';
  }
}
