...
export class StepConfiguration implements OnInit {
  ...
  ngOnInit(): void {
    if (this.group) {
      ...
      if (taxControl) {
        taxControl.addAsyncValidators(
          AsyncValidators.createTaxIdValidator(this.http, this.apiUrl),
        );
            ...
            if (this.parentForm) {
              this.parentForm.patchValue({
                business: {
                  ...
                  industry:
                    data.industry ||
                    this.parentForm.get('business.industry')?.value,
                },
              });
            }
          } else {
  ...
  getTaxIdError(): string {
    ...
    if (control?.touched && control?.errors) {
      ...
      if (control.errors['pattern'])
        return (
          this.countryService.currentCountry()?.formSchema?.taxId
            ?.errorMessage || 'REGISTER.ERRORS.INVALID_FORMAT'
        );
