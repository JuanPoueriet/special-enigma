...
@Injectable()
export class MikroOrmAccountsPayableRepository implements AccountsPayableRepository {
  constructor(private readonly em: EntityManager) {}

  async findAll(tenantId: string): Promise<Invoice[]> {
    return this.em.find(Invoice, { tenantId, type: 'PAYABLE' });
  }

  async findByVendor(tenantId: string, vendorId: string): Promise<Invoice[]> {
