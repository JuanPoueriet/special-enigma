...
import {
  GetAccountsUseCase,
} from '../use-cases/accounts/get-accounts.use-case';
import {
  GetJournalEntriesUseCase,
} from '../use-cases/journal-entries/get-journal-entries.use-case';
import {
  CountJournalEntriesUseCase,
} from '../use-cases/journal-entries/count-journal-entries.use-case';
import {
  GenerateFinancialReportUseCase,
} from '../use-cases/reports/generate-financial-report.use-case';
import { FinancialReportType } from '@virteex/domain-accounting-contracts';
import { JOURNAL_ENTRY_REPOSITORY, JournalEntryRepository, AccountType } from '@virteex/domain-accounting-domain';
...
export class AccountingQueryFacade {
  ...
  async generateFinancialReport(tenantId: string, type: FinancialReportType, endDate: Date, dimensions?: Record<string, string>) {
    return this.generateFinancialReportUseCase.execute(tenantId, type as any, endDate, dimensions);
  }
  ...
  async getMonthlyOpex(tenantId: string): Promise<number> {
    ...
    const balances = await this.journalEntryRepository.getBalancesByAccount(tenantId, firstDayOfMonth, now);
    ...
    for (const [_, balance] of balances) {
      ...
      totalOpex = totalOpex.plus(new Decimal(balance.debit).minus(new Decimal(balance.credit)));
    }
