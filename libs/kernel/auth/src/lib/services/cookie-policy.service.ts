...
export class CookiePolicyService {
  ...
  setAuthCookies(
    ...
    res.cookie(
      'access_token',
      accessToken,
      buildAccessCookieOptions(cookieContext),
    );
    res.cookie(
      'refresh_token',
      refreshToken,
      buildRefreshCookieOptions(cookieContext, rememberMe),
    );
  }
  ...
  getCookieContext() {
    const isProd =
      this.secretManager?.getSecret('NODE_ENV', 'development') ===
        'production' || process.env['NODE_ENV'] === 'production';

    if (isProd) {
      const sessionSecret = this.secretManager?.getSecret('SESSION_SECRET', '');
      if (!sessionSecret) {
        throw new CriticalConfigurationException(
          'SESSION_SECRET must be defined in production environment',
        );
      }
    }

    const secure =
      this.secretManager?.getSecret('COOKIE_SECURE', String(isProd)) === 'true';
    const sameSite =
      (this.secretManager?.getSecret('COOKIE_SAME_SITE', 'lax') as
        | 'lax'
        | 'strict'
        | 'none') || 'lax';
