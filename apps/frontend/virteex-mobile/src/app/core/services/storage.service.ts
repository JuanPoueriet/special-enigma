...
export class StorageService {
  ...
  private initDB(): Promise<IDBDatabase> {
    if (typeof indexedDB === 'undefined') {
      this.logger.warn(
        'IndexedDB not supported, falling back to memory/localstorage might be needed',
      );
  ...
  async get<T>(key: string): Promise<T | null> {
    try {
      ...
      this.logger.error('Storage Get Error', e);
      return null;
    }
  }
  ...
  async set(key: string, value: any): Promise<void> {
    try {
      ...
      this.logger.error('Storage Set Error', e);
      throw e;
    }
  }
  ...
  async remove(key: string): Promise<void> {
    try {
      ...
      this.logger.error('Storage Remove Error', e);
      throw e;
    }
  }
}
